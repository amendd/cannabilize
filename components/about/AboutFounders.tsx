import OptimizedImage from '@/components/ui/OptimizedImage';

const FOUNDER = {
  name: 'Edson Neto',
  role: 'Fundador da Cannabilize',
  photo: '/images/founders/edson-neto.png',
  story: {
    problem: 'Muita gente quer acesso ao tratamento com cannabis medicinal, mas não sabe por onde começar ou tem medo de processos confusos e pouco transparentes.',
    motivation: 'Acreditamos que cada pessoa merece um caminho claro, seguro e acompanhado por quem entende.',
    mission: 'Criamos a Cannabilize para que você tenha da consulta à documentação um processo humanizado — e que ver alguém recuperar qualidade de vida continue nos movendo todos os dias.',
    closing: 'Hoje seguimos trabalhando para que cada paciente tenha acesso seguro, orientação clara e acompanhamento verdadeiro durante todo o tratamento.',
  },
};

export default function AboutFounders() {
  return (
    <section className="py-14 md:py-20 bg-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <p className="text-sm font-semibold text-primary-600 uppercase tracking-wider mb-3 md:mb-4">
          Quem está por trás
        </p>
        <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-gray-900 mb-8 md:mb-12 font-display tracking-tight">
          Fundador
        </h2>

        <div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-10 lg:gap-12">
          <div className="shrink-0 w-44 h-44 sm:w-52 sm:h-52 md:w-60 md:h-60 lg:w-64 lg:h-64 rounded-2xl overflow-hidden border-4 border-primary-100 shadow-xl bg-gray-100 ring-2 ring-white">
            <OptimizedImage
              src={FOUNDER.photo}
              alt={`Foto de ${FOUNDER.name}`}
              width={256}
              height={256}
              className="w-full h-full object-cover"
              fallback="/images/team/default-avatar.jpg"
            />
          </div>
          <div className="flex-1 min-w-0 text-center md:text-left">
            <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-1">{FOUNDER.name}</h3>
            <p className="text-primary-600 font-semibold text-sm md:text-base mb-5">{FOUNDER.role}</p>
            <div className="space-y-5 text-gray-600 leading-relaxed text-sm md:text-base">
              <p>
                <strong className="text-gray-800">O que vimos:</strong> {FOUNDER.story.problem}
              </p>
              <p>
                <strong className="text-gray-800">O que nos move:</strong> {FOUNDER.story.motivation}
              </p>
              <p>
                <strong className="text-gray-800">Por que a Cannabilize existe:</strong> {FOUNDER.story.mission}
              </p>
              <p className="pt-1 text-gray-700">
                {FOUNDER.story.closing}
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
